...
3. Виправлення: лінійна інтерполяція аномальних точок на основі сусідніх коректних точок.
Якщо аномальна точка знаходиться на початку або в кінці масиву, або якщо навколо неї немає достатньої кількості коректних точок для інтерполяції, її слід залишити як є або видалити (обери безпечніший підхід).
4. Вихід: JSON з виправленими точками та статистикою (кількість виявлених/виправлених аномалій, можливо, середня швидкість).
Код повинен бути читабельним, ефективним та обробляти можливі помилки вводу/виводу, некоректний JSON.
Використовуй стандартні бібліотеки C++. Не використовуй сторонні бібліотеки для парсингу JSON, напиши свій мінімальний парсер або використовуй `nlohmann/json` (якщо це прийнятно для середовища, але краще без сторонніх залежностей для простоти компіляції).
Якщо ти не можеш обійтися без сторонніх залежностей, вкажи це явно в коментарі на початку коду та надай інструкцію з встановлення.
Використовуй формулу Гаверсина для розрахунку відстані між GPS координатами.
Надай лише готовий C++ код у блоці markdown, без зайвих пояснень поза блоком коду.
"""
        messages = [
